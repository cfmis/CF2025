...
namespace CF2025.Web.Admin.Areas.Store.Controllers
{
    public class ChangeStoreController : Controller
    {
        ...
        {
            var result = IssueAppriseListDAL.Save(headData, lstDetailData1, lstDetailData2, lstDelData1, lstDelData2, user_id);
        ...
        //public JsonResult GetLotNoList(string location_id, string mo_id, string goods_id)
        //{
        //    var result = IssueAppriseListDAL.GetLotNoList(location_id, mo_id, goods_id);
        //    return Json(result, JsonRequestBehavior.AllowGet);
        //}
        ...
        {
            var result = IssueAppriseListDAL.CheckStorage(id);
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult Approve(st_inventory_mostly head, string user_id, string approve_type)
        {
            var result = "";
            //批準
            result = IssueAppriseListDAL.ApproveChangeStore(head, user_id, approve_type); 
            if (result.Substring(0, 2) == "00")
            {
                result = "OK";
            }
            ...
            //var result = IssueAppriseListDAL.ApproveChangeStore(head, user_id, approve_type);
            ////return Json(result, JsonRequestBehavior.AllowGet);
            //if (result == "")
            //    return Json("OK");
            //else
        ...
        {
            string result = IssueAppriseListDAL.DeleteHead(head);
            if (result == "")
                return Json("OK");
            else
                return Json("Error");
