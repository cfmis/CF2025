...
var Main = {
    data() {
        return {
            ...
            tableHeight1 :400,           
            btnItemTitle:"劉覽",           
            headData: { id: '', inventory_date: '', origin: '1', state: '0',bill_type_no:'01',department_id:'',ii_location:'',linkman:'',handler:'',remark: '',create_by: '', 
                create_date: '', update_by: '',update_date: '',check_by: '', check_date: '', tum_type:'D',update_count:'1',transfers_state:'0',
                servername:'hkserver.cferp.dbo',head_status:'' },
            rowDataEdit1: { id:'',sequence_id:'',mo_id:'',goods_id:'',goods_id_new:'',goods_name:'',goods_name_new:'',inventory_issuance:'',ii_code:'',ir_lot_no:null,obligate_mo_id:'',
                i_amount:0,i_weight:0,inventory_receipt:'',ir_code:'',ii_lot_no:'',ref_lot_no:'',ib_qty:0,ib_weight:0,unit:'PCS',remark:null,
                ref_id:'', jo_sequence_id:'', so_no:null,contract_cid:null,mrp_id:null, sign_by:null,sign_date:null, vendor_id:'',vendor_name:'',base_unit:'PCS',rate:1,state:'0',
                transfers_state:'0',ref_sequence_id:'',only_detail:'1',average_cost:0,sec_unit:'KG',row_status:'' },           
            formRules: {
                id: [{ required: true},{ min: 13, message: '請度13个字符' }],
                inventory_date:[{ required: true, message: '請輸入轉倉日期' }],
                ii_location:[{ required: true, message: '請輸入轉出倉' }]
            },
            rulesRowEdit1: {
            ...
            tableGoodsList: [{goods_id:'',goods_name:'',lot_no:'',qty:0.00,sec_qty:0.00,mo_id:'',vendor_id:'',vendor_name:'',average_cost:0}],
    ...
    created() {
        ...
        this.getComboxList("LocationList");//倉庫,貨架,由選擇的倉庫帶出，轉出倉       
    ...
    methods: {
        ...
        clearRowDataEdit1(){
            this.rowDataEdit1= {
                id:'',sequence_id:'', mo_id: '', goods_id: '',goods_id_new: '', goods_name: '',goods_name_new: '',inventory_issuance:'',ii_code:'',ir_lot_no:null,obligate_mo_id:'',
                i_amount:0,i_weight:0,inventory_receipt:'',ir_code:'',ii_lot_no:'',ref_lot_no:'',ib_qty:0,ib_weight:0,unit:'PCS',remark:null,
                ref_id:'', jo_sequence_id:'', so_no:null,contract_cid:null,mrp_id:null, sign_by:null,sign_date:null, vendor_id:'',vendor_name:'',base_unit:'PCS',
                rate:1,state:'0',transfers_state:'0',ref_sequence_id:'',only_detail:'1',average_cost:0,sec_unit:'KG',row_status:'' }
        },
        setRowDataEdit1(curRow) {
            this.rowDataEdit1={
                id:curRow.id,sequence_id:curRow.sequence_id, mo_id: curRow.mo_id, goods_id: curRow.goods_id,goods_id_new: curRow.goods_id_new,goods_name: curRow.goods_name,
                goods_name_new: curRow.goods_name_new,inventory_issuance:curRow.inventory_issuance,ii_code:curRow.inventory_issuance,ir_lot_no:curRow.ir_lot_no,obligate_mo_id:curRow.obligate_mo_id,
                i_amount:curRow.i_amount,i_weight:curRow.i_weight,inventory_receipt:curRow.inventory_receipt,ir_code:curRow.inventory_receipt,
                ...
                sign_date:curRow.sign_date,vendor_id:curRow.vendor_id,vendor_name:curRow.vendor_name,base_unit:curRow.base_unit,rate:1,state:'0',
                transfers_state:'0',ref_sequence_id:curRow.ref_sequence_id,only_detail:1,average_cost:0,sec_unit:'KG',row_status:curRow.row_status }
        },
        backupData () {
        ...
        insertRowEvent() {
            ...
            if (this.headData.ii_location == "") {
                this.$XModal.alert({ content: '主檔中的轉出倉不可為空,當前操作無效!', mask: false });
                return;
            };
        ...
        saveEvent(){           
            ...
            if(this.headData.ii_location ===""){
                this.$XModal.alert({ content: '轉出倉不可以為空!', mask: false });
                return;
            }
        ...
        checkMo:async function(mo_id) {            
            ...
            if(returnMoId==="" && mo_id==="Z99999999"){
                returnMoId="Z99999999";
            }
            if(returnMoId ===""){
        ...
        approveEvent:async function (val) {           
            if ((this.headData.id != "") && (this.gridData1.length > 0)) {
                ...
                if(val==='0'){ 
                    ...
                    //}
                    //檢查已批準日期是否為當日,超過當日則不可反批準
                    let isApprove = await comm.canApprove(this.headData.id,"w_changestore_cd");//轉貨品編號/轉批號
                    if(isApprove ==="0"){
                        this.$XModal.alert({ content: "注意:【批準日期】必須為當前日期,方可進行此操作!", mask: false });
                        return;
                    }                    
        ...
        setModify() {           
            ...
            this.$set(this.rowDataEdit1,"sec_unit","KG");
            this.$set(this.rowDataEdit1,"inventory_issuance",this.headData.ii_location);
            this.$set(this.rowDataEdit1,"ii_code",this.headData.ii_location);
            this.$set(this.rowDataEdit1,"inventory_receipt",this.headData.ii_location);
            this.$set(this.rowDataEdit1,"ir_code",this.headData.ii_location);
            Object.assign(this.selectRow, this.rowDataEdit1);
            this.showEdit = false;
        },
        //**雙擊表格單元格時觸發的事件，暫未用到
        ...
        getHead(id) {
            ...
            axios.get("/ChangeStoreZc/GetHeadByID", { params: { id: id , moduleType: "D"}}).then(
                (response) => {
                    this.headData = {
                        id: response.data.id,
                        ...
                        ii_location: response.data.ii_location
                    }
        ...
        saveAll: async function() {
            ...
            if(flagMo ===true){
                this.$XModal.alert({ content: `第【${li_row}】行,頁數與庫存頁數必須保持一致!`,status: 'info' , mask: false });
                return;
            }
            flagMo = false;
            //當貨品編號與新貨品編號不同（轉貨品編號），要檢查批號必須相同，頁數與庫存頁數必須相同
            for(let i=0;i<$table.tableData.length;i++){
                items_update = $table.tableData[i];
                if(items_update.goods_id != items_update.goods_id_new ){
                    if(items_update.ii_lot_no != items_update.ir_lot_no){
                        flagMo = true;
                        li_row = i+1;
                        break;
                    }
            ...
            if(flagMo ===true){
                this.$XModal.alert({ content:  `第【${li_row}】行,轉貨品編號時,批號與新批號必須保持一致!`,status: 'info' , mask: false });
                return;
            }
            //當貨品編號與新貨品編號相（轉批號），要檢查批號與新批號不能夠相同，頁數與庫存頁數必須相同
            for(let i=0;i<$table.tableData.length;i++){
                items_update = $table.tableData[i];
                if(items_update.ii_lot_no != items_update.ir_lot_no){
                    if(items_update.goods_id != items_update.goods_id_new){
            ...
            if(flagMo ===true){
                this.$XModal.alert({ content:  `第【${li_row}】行,轉批號時,貨品編號與新貨品編號必須保持一致!`,status: 'info' , mask: false });
                return;
            }
            ...
            if(this.tempSaveData.length>50){
                let len = this.tempSaveData.length;
                this.$XModal.alert({ content:  `當前資料記錄數為:${len},已超出範圍，當前操作無效!`,status: 'info' , mask: false });
                return
            }
        ...
        checkStock:async function(id){            
            let rtn="";           
            await axios.post("/ChangeStoreZc/CheckStock",{ id:id,moduleType:"D" }).then((response) => {
        ...
        showGoods:async function(event,type){
            if(this.headData.state ==='0') {//可編輯狀態時
                this.flagClick = type;
                let mo_id = this.rowDataEdit1.mo_id;
                let ii_location = this.headData.ii_location
                await axios.get("/ChangeStoreCd/GetPlanItemList",{ params: { mo_id:mo_id,ii_location:ii_location } } ).then(
                    (response) => {
                ...
                let $pulldown = (type===1)? this.$refs.pulldownRef1:this.$refs.pulldownRef2;
                if ($pulldown) {
        ...
        showLotNo:async function(event,type){
            if(this.headData.state ==='0') {//單元格為可編輯狀態時                
                let location_id = this.headData.ii_location;//  this.rowDataEdit1.inventory_issuance;
                let goods_id = this.rowDataEdit1.goods_id;
                let mo_id = this.rowDataEdit1.mo_id;
                this.flagClickLotNo = type;
                if(this.$utils.isEmpty(goods_id)){
                    //this.$refs.xTable2.setActiveCell(this.selectRow2, "goods_id");
                    return;
                }
                //**R單轉正單，提取批号数据 //批號下拉列表框
                await axios.get("/Base/Common/GetStockLotNoConvert", { params: { location_id:location_id,goods_id:goods_id,mo_id:mo_id } }).then(
                    (response) => {
                ...
                let $pulldown = (type===1)? this.$refs.pulldownLotNoRef1:this.$refs.pulldownLotNoRef2;
                if ($pulldown) {
                    $pulldown.togglePanel();                   
                }
            }
        ...
        getMaxID:async function() {
            //let $table = this.$refs.xTable1;
            await axios.get("/Base/Common/GetMaxIDStock?bill_id=" + 'ST11' + "&serial_len=4" ).then(
                (response) => {
        ...
        //**顯示生產計劃流程中的貨品編碼列表
        cellGoodsClick(row){
            let $pulldown = (this.flagClick ===1)? this.$refs.pulldownRef1:this.$refs.pulldownRef2;
            if ($pulldown) {
                let rowIndex = row.$rowIndex;
                let goods_id = row.data[rowIndex].goods_id;
                let goods_name = row.data[rowIndex].goods_name;               
                this.$set(this.rowDataEdit1,"vendor_id",row.data[rowIndex].vendor_id );
                this.$set(this.rowDataEdit1,"vendor_name",row.data[rowIndex].vendor_name );
                this.$set(this.rowDataEdit1,"average_cost",row.data[rowIndex].average_cost );
                if(this.flagClick ===1){
                    this.$set(this.rowDataEdit1,"goods_id",goods_id );
                    this.$set(this.rowDataEdit1,"goods_name",goods_name );
                    this.$set(this.rowDataEdit1,"goods_id_new",goods_id );
                    this.$set(this.rowDataEdit1,"goods_name_new",goods_name );
                    this.$set(this.rowDataEdit1,"i_amount",row.data[rowIndex].qty );
                    this.$set(this.rowDataEdit1,"i_weight",row.data[rowIndex].sec_qty );
                    this.$set(this.rowDataEdit1,"ii_lot_no",row.data[rowIndex].lot_no );
                    this.$set(this.rowDataEdit1,"ir_lot_no",row.data[rowIndex].lot_no );
                }else{
                    this.$set(this.rowDataEdit1,"goods_id_new",goods_id );
                    this.$set(this.rowDataEdit1,"goods_name_new",goods_name );
                }
        ...
        //**顯示是庫存中的批號列表
        cellLotNoClick(row){   
            //ii_lot_no 原批號;ir_lot_no新批號
            let $pulldown =(this.flagClickLotNo ===1)?this.$refs.pulldownLotNoRef1:this.$refs.pulldownLotNoRef2;
            if ($pulldown) {
                let rowIndex = row.$rowIndex;
                if(this.flagClickLotNo ===1){
                    this.$set(this.rowDataEdit1,"i_amount",row.data[rowIndex].qty );
                    this.$set(this.rowDataEdit1,"i_weight",row.data[rowIndex].sec_qty );
                    this.$set(this.rowDataEdit1,"ii_lot_no",row.data[rowIndex].lot_no );
                    this.$set(this.rowDataEdit1,"ir_lot_no",row.data[rowIndex].lot_no );
