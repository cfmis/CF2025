...
var main ={
    data()  {
        return {
            ...
            validRules: {
                ...
                location: [{ required: true, message: '倉庫不可為空'}],
                //qty: [{ type: 'number', min: 1, max: 10000000, message: '數量不可為空(輸入範圍1~10000000)'}],
                unit: [{ required: true, message: '單位不可為空' }], 
    ...
    created() {
        this.getComboxList("AdjustmentReason");//調整原因
    ...
    methods: {
            ...
            if(dept_id != this.selectRow.location){
                this.$XModal.confirm('更改主表倉庫后,會同時將明細表中所有數據的倉庫都更改？').then(type => {
                    if (type == 'confirm') {
                        ...
                        this.headData.department_id = this.selectRow.location;
                    }
                })
            }
        },
        ...
        insertEvent() {
            ...
            this.isDisableDept = false; //可編輯
            this.isDisable = false; //下拉列表可編輯
        ...
        revertEvent() {
            ...
            this.headEditFlag = "";
            this.isDisableDept = true; //不可編輯
            this.isDisable = true;//不可編輯
        ...
        clearRowDataEdit(){
            this.rowDataEdit = {id:'',sequence_id:'',mo_id:'',goods_id:'',goods_name:'',color:'',location:'',carton_code:'',unit:'PCS',
                qty:0.00,ib_amount:0.00,price:0.00,transfers_state:'0',sec_unit:'KG',sec_qty:0.00,ib_weight:0.00,lot_no:'',remark:'',row_status:''}
        },
        backupData () {
        ...
        insertRowEvent() {
            ...
            if (this.headData.department_id === "") {
                ...
                this.$message({
                    ...
                    offset: window.screen.height / 2 - 200
                });
            ...
            this.$set(this.rowDataEdit, "sequence_id", seq_id);//this.getSequenceId($table.tableData.length)
            this.$set(this.rowDataEdit, "location", this.headData.department_id);
        ...
        editRowEvent(row){
            ...
            this.isReadOnly = (this.headData.state==='0')?false:true; 
            this.isDisable = (this.headData.state==='0')?false:true; 
        ...
        getMaxID() {
            ...
            axios.get("/Base/Common/GetMaxIDStock?bill_id=" + 'ST02' + "&serial_len=4" ).then(
                (response) => {
                    ...
                    //    this.$set($table.tableData[i], "location", this.headData.department_id );
                    //    this.$set($table.tableData[i],"carton_code",this.headData.department_id);
                    //}
                }
            );
        }, 
        ...
        approveEvent:async function(val) {
            ...
            if ((this.headData.id != "") && (this.tableData1.length > 0)) {
                ...
                //取后端單據狀態
                status = await comm.checkApproveState('st_adjustment_mostly',this.headData.id);
                if(status==='2'){
                ...
                this.$XModal.confirm(ls_approve).then(type => {
                     if (type === "confirm") {
                            ...
                            axios.post("/Adjustment/Approve",{head,user_id,approve_type}).then(
                                (res) => {
                                    if(res.data ==="OK"){
                                        this.setStatusHead(false);
                                        //重查刷新數據
        ...
        dbclickEvent(row){
            ...
            this.isDisable = (this.headData.state ==='0')?false:true;//2023/09/22
        ...
        cellClickEvent(row){
            ...
            this.showGoodsX = row.$event.clientX-180; 
        ...
        /*type:'1'表格中的調用,'2'為編輯彈窗中的調用 */
        showLotNo(event,type){
            if(this.headData.state==='0') {//單元格為可編輯狀態時
                this.seledtLotNoRow = null;
                if(type=='2'){
                    this.showGoodsX = event.clientX-725;
                    this.showGoodsY = event.clientY-20;
                }else{
        ...
        getHead:async function(id) {           
            ...
            await axios.get("/Adjustment/GetHeadByID", { params: { id: id }}).then(
            //await axios.post("/ProduceAssembly/GetHeadByID", { id: id }).then(
                (response) => {                
                    this.headData = {
                        ...
                        adjust_reason: response.data.adjust_reason,
                    ...
                    this.isDisable = (response.data.state==='0')?false:true;
                    this.btnItemTitle = (this.headData.state==='0')?'修改':'劉覽';
        ...
        getDetails:async function(id) {
            await axios.get("/Adjustment/GetDetailsByID", { params: { id: id }  }).then(
                (response) => {
        ...
        //批號下拉列表框
        getLotNoData(goods_id,dept_id,mo_id){
            if(this.$utils.isEmpty(goods_id)){
                this.$refs.xTable1.setActiveCell(this.selectRow, "goods_id");                
                return;
            }
            axios.get("/ProduceAssembly/GetLotNoList", { params: { goods_id: goods_id,location_id:dept_id,mo_id:mo_id } }).then(
                (response) => {
        ...
        changeItem() {
            if(this.selectFindItemRow) {
                if(this.selectRow.goods_id != this.selectFindItemRow.goods_id){
                    ...
                    this.$set(this.selectRow,"lot_no",this.selectFindItemRow.lot_no);                    
                    this.$set(this.selectRow, "ib_amount", this.selectFindItemRow.qty);
                    this.$set(this.selectRow, "ib_weight", this.selectFindItemRow.sec_qty);
                    this.setCancelStatus();
        ...
        changeLotNo() {
            if(this.seledtLotNoRow){
                if(this.selectRow.lot_no != this.seledtLotNoRow.lot_no){
                    ...
                    this.$set(this.selectRow, "ib_amount", this.seledtLotNoRow.qty);
                    this.$set(this.selectRow, "ib_weight", this.seledtLotNoRow.sec_qty);
        ...
        saveAllEvent: async function() {            
            if(this.headData.id==="" && this.headData.out_dept===""){                
                this.$XModal.alert({ content: '倉庫不可為空!', mask: false });//mask: false 彈窗時父窗口不需庶罩
                return;
            }
            if(this.headData.date ===""){
                this.$XModal.alert({ content: '調整日期不可為空!', mask: false });
                return;
            }
    ...
    checkPreUpdate:async function() {
            let	ls_id='',ls_goods_id='',ls_dept,ls_mo_id='',li_row_no=0;                    
            ls_dept = this.headData.department_id;           
            ls_id = this.headData.id;
            this.preUpdateFlag = true;
            if(ls_dept===""){
                await this.$XModal.alert({ content: `倉庫不可為空!`,status: 'warning' , mask: false });
            ...
            for(let li_row=0;li_row<this.tableData1.length;li_row++) {
                ...
                if(ls_lot_no===""){
                    await this.$XModal.alert({ content: `第 ${li_row_no} 行,批號不可以為空!`,status: 'warning' , mask: false });
                    this.preUpdateFlag = false;
                    break;
                }
                ls_mo_id = this.tableData1[li_row].mo_id;
                if(ls_mo_id===""){
                    await this.$XModal.alert({ content: `第 ${li_row_no} 行,頁數不可以為空!`,status: 'warning' , mask: false });
                    this.preUpdateFlag = false;
                    break;
                }
                ls_goods_id=this.tableData1[li_row].goods_id;
                if(ls_goods_id===""){
                    await this.$XModal.alert({ content: `第 ${li_row_no} 行,貨品編碼不可以為空!`,status: 'warning' , mask: false });
                    this.preUpdateFlag = false;
                    break;
                }
                ls_lot_no=this.tableData1[li_row].unit;
                if(ls_lot_no===""){
                    await this.$XModal.alert({ content: `第 ${li_row_no} 行,單位不可以為空!`,status: 'warning' , mask: false });
                    this.preUpdateFlag = false;
                    break;
                }
                ls_lot_no=this.tableData1[li_row].sec_unit;
                if(ls_lot_no===""){
                    await this.$XModal.alert({ content: `第 ${li_row_no} 行,重單位不可以為空!`,status: 'warning' , mask: false });
                    this.preUpdateFlag = false;
                    break;
                }
                ls_lot_no=this.tableData1[li_row].location;
                if(ls_lot_no===""){
                    await this.$XModal.alert({ content: `第 ${li_row_no} 行,明細倉庫不可以為空!`,status: 'warning' , mask: false });
                    this.preUpdateFlag = false;
                    break;
                }
                if(ls_lot_no != ls_dept){
                    await this.$XModal.alert({ content: `第 ${li_row_no} 行,主表與明細的倉庫必須保持一致!`,status: 'warning' , mask: false });
                    this.preUpdateFlag = false;
                    break;
                }
                //wf_check_inv
