...
var main={
    data() {
        return {                       
            ...
            btnItemTitle:'劉覽',
            formData: { jo_id_s: '',jo_id_e: '',mo_id_s:'',mo_id_e:'',goods_id_s:'',goods_id_e:'', charge_dept_s:'',charge_dept_e:'',location_id:'',four_color:'',
                production_date_s:'', production_date_e:'',check_date_s:'',check_date_e:'' },
            ...
            //    so_sequence_id:'',ref_sequence_id:'',inventory_date:null,department_id:null,only_detail:'0',
            //    vendor_id:null,servername:null,row_status:''},
    ...
    methods: {
        ...
        //}},
        resetEvent(){
            this.formData.jo_id_s="";
            this.formData.jo_id_e="";
            this.formData.goods_id_s="";
            this.formData.goods_id_e="";
            this.formData.mo_id_s="";
            this.formData.mo_id_e="";
            this.formData.charge_dept_s="";
            this.formData.charge_dept_e="";
            this.formData.location_id="";
            this.formData.production_date_s="";
            this.formData.production_date_e="";
            this.formData.check_date_s="";
            this.formData.check_date_e="";
        },
        queryData(){
            this.loading1 = true;
            var query = JSON.parse(JSON.stringify(this.formData));
            axios.post("/IssueAppriseList/QueryData",{query}).then(
                (response) => {
        ...
        */
        ...
        insertEvent:async function() {                     
            var $table = this.$refs.xTable1;
            if ($table) {
                let aryRecord = $table.getCheckboxRecords();
                if(aryRecord.length ==0){
                    this.$XModal.alert("請首先選中要發料的貨品!");//模式提示窗
                    return;
                }               
                this.tempAddData1=[];
                this.tempAddData2=[];
                for( let i=0;i<aryRecord.length;i++){
                    this.tempAddData1.push(aryRecord[i]);
                    await this.addDataToTable2(aryRecord[i].location,aryRecord[i].carton_code,aryRecord[i].mo_id,aryRecord[i].materiel_id,aryRecord[i].upper_sequence,aryRecord[i].key_id);
                }                
                
                //this.$XModal.alert(`选中：${selectRecords.length}`);//模式提示窗
                //VXETable.modal.alert(`选中：${selectRecords.length}`);//模式提示窗

               
                $table.removeCheckboxRow();//移除父表格中選中的記錄
                var url = "/Store/ChangeStore?flagChild=" + "1";
                comm.showMessageDialog(url, "倉庫發料", 1280, 768, true);
            }
        ...
        clearFormData(){
            this.formData.id="",
            this.formData.con_date = "",
            this.formData.bill_origin = "2",
            this.formData.out_dept = "",
            this.formData.in_dept = "",
            this.formData.handover_id ="",
            this.formData.handler = "",
            this.formData.remark = "",
            this.formData.create_by = "",
            this.formData.update_by = "",
            this.formData.check_by = "",
            this.formData.update_count = "",
            this.formData.create_date = "",
            this.formData.update_date = "",
            this.formData.check_date = "",
            this.formData.state ="0" 
            this.formData.head_status ="";
        },
        clearRowDataEdit(){
        ...
        approveEvent:async function(val) {
            ...
            if ((this.headData.id != "") && (this.tableData1.length > 0)) {
                ...
                //取后端單據狀態
                let status = comm.checkApproveState('jo_assembly_mostly',this.headData.id);               
                if(status==='2'){
                ...
                if(val==='0'){                    
                    ...
                    //檢查已批準日期是否為當日?,超過當日則不可反批準
                    var isApprove = await comm.canApprove(this.headData.id,"w_produce_assembly");//組裝转換單
                    if(isApprove ==="0"){
                        this.$XModal.alert({ content: "注意:【批準日期】必須為當前日期,方可進行此操作!", mask: false });
                        return;
                    }
                ...
                this.$XModal.confirm(ls_is_approve).then(type => {
                    if (type == "confirm") {
                        ...
                        this.showButtonApprove(approve_type,"1");//禁止顯示反批準/批準按鈕
        ...
        cellClickEvent(row){
            ...
            this.setTableData2(this.selectRow.location, this.selectRow.carton_code, this.selectRow.mo_id, this.selectRow.materiel_id, this.selectRow.upper_sequence,this.selectRow.key_id);
        },
        //點擊表格一時表格二跟著變化
        setTableData2 (location,carton_code,mo_id,materiel_id,upper_sequence,key_id) {
            this.tableData2 =[];           
            this.loading2 = true;
            axios.post("/IssueAppriseList/QueryDataSub",{ location_id:location,carton_code:carton_code,mo_id:mo_id,materiel_id:materiel_id,upper_sequence:upper_sequence,key_id:key_id }).then(
                (response) => {
        ...
        addDataToTable2:async function(location,carton_code,mo_id,materiel_id,upper_sequence,key_id){//添加發料明細表2            
            await axios.post("/IssueAppriseList/QueryDataSub",{ location_id:location,carton_code:carton_code,mo_id:mo_id,materiel_id:materiel_id,upper_sequence:upper_sequence,key_id:key_id }).then(
                (response) => {                                     
                    if(response.data.length>0){
                        for(let i=0;i<response.data.length;i++){
                            this.tempAddData2.push(response.data[i]);
                        }
                    }
                }
            ).catch(function (response) {
        ...
        getHead:async function(id) {           
            ...
            await axios.get("/ProduceAssembly/GetHeadByID", { params: { id: id }}).then(
        ...
        getDetailsPart:async function (id) {                       
           await axios.get("/ProduceAssembly/GetDetailsPartByID", { params: { id: id}  }).then(
                (response) => {
                    ...
                    this.setTableData2(); //從臨時變量中找出對應的數據出來填充表格2,默認表格1中的第一行                    
                }
            ).catch(function (response) {
                alert(response);
            });
        ...
        saveAllEvent: async function() {
            if(this.headData.id ==="" ){                
                this.$XModal.alert({ content: '編號不可為空!', mask: false });//mask: false 彈窗時父窗口不需庶置
                return;
            }
        ...
        checkRechangeApproveStatus:async function(){           
            await axios.post("/ProduceAssembly/CheckRechangeStatus",{within_code:'0000',handover_id:this.headData.handover_id, con_date:this.headData.con_date,id:this.headData.id,state:'1'}).then(
                (response) => {
    ...
    watch: {
        formData:{
            handler (newValue, oldValue){
                this.formData.mo_id_s = this.formData.mo_id_s.toUpperCase();
                this.formData.mo_id_e = this.formData.mo_id_e.toUpperCase();
                this.formData.jo_id_s = this.formData.jo_id_s.toUpperCase();
                this.formData.jo_id_e = this.formData.jo_id_e.toUpperCase();
                this.formData.goods_id_s = this.formData.goods_id_s.toUpperCase();
                this.formData.goods_id_e = this.formData.goods_id_e.toUpperCase();
                this.formData.charge_dept_s = this.formData.charge_dept_s.toUpperCase();
                this.formData.charge_dept_e = this.formData.charge_dept_e.toUpperCase();                
            },
            deep: true
        },       
