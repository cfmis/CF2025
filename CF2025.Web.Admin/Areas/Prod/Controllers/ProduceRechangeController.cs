...
{
    public class ProduceRechangeController : AdminControllerBase //Controller
    {
        // 移交單    
        ...
        {
            var DataHead = ProduceRechangeDAL.GetHeadByID(id);
            return Json(DataHead, JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetDetailsByID(string id)
        {
            var DataDetail = ProduceRechangeDAL.GetDetailsByID(id);
        ...
        {
            var result = ProduceRechangeDAL.CheckStorage(id);
        ...
        public JsonResult Approve(jo_assembly_mostly head, string user_id, string approve_type)
        {
            var result = "";
            //批準/反批準  
            result = ProduceRechangeDAL.Approve(head, user_id, approve_type);           
        ...
        {
            var result = ProduceRechangeDAL.GetTotalRechange(id, mo_id, goods_id,out_dept,in_dept);            
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var result = ProduceRechangeDAL.GetShadingColorData(out_dept,mo_id,goods_id);
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var result = ProduceRechangeDAL.CheckPlanQcProcess(mo_id, goods_id,out_dept,in_dept);
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var result = ProduceRechangeDAL.CheckOcQcProcess(mo_id, goods_id, out_dept, in_dept);
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var result = ProduceRechangeDAL.CheckRechangeQc(mo_id, goods_id, out_dept, in_dept);
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var result = ProduceRechangeDAL.Save(headData, lstDetailData1, lstDelData1, user_id);
            if (result == "")
                result = "OK";
            else
                result = "Error";
        ...
        {
            string result = ProduceRechangeDAL.DeleteHead(head, user_id);
            if (result == "")
                return Json("OK");
            else
                return Json("Error");
