...
namespace CF.Account.BLL
{
    public class AccountService : IAccountService
    {
        ...
        public LoginInfo GetLoginInfo(Guid token)
        {
            return CacheHelper.Get<LoginInfo>(string.Format(_LoginInfoKeyFormat, token), () =>
            {
                using (var dbContext = new AccountDbContext())
                {
                    ...
                    //var defTime = EntityFunctions.DiffMinutes(DateTime.Now, DateTime.Now.AddMinutes(3));
                    //var defTime1 = DbFunctions.DiffMinutes(DateTime.Now, DateTime.Now.AddMinutes(3));
        ...
        public LoginInfo Login(string loginName, string password,string languageid)
        {
            ...
            using (var dbContext = new AccountDbContext())
            {
                ...
                if (user != null)
                {
                    ...
                    else
                    {
                        ...
                        loginInfo.LanguageID = languageid;
