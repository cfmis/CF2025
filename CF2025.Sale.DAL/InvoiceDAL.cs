...
namespace CF2025.Sale.DAL
{
    public static class InvoiceDAL
    {
        ...
        public static viewInv GetInvoiceByID(string ID)
        {
            ...
            return lsInv;
            //var mdjDetails = ConvertHelper.convertToList<so_invoice_details1>(dt);
            //viewOc1 mdj = new viewOc1();
            //mdj.ocMostly = mdjMostly;
            //mdj.ocDetails = mdjDetails;
            //for (int i = 0; i < dt.Rows.Count; i++)
            //{
            //    DataRow dr = dt.Rows[i];
            ...
            //    mdj.ocMostly.oi_date = Converter.ConvertFieldToCnDataString(dr["oi_date"].ToString());
            ...
            //    mdj.ocMostly.payment_date = Converter.ConvertFieldToCnDataString(dr["payment_date"].ToString());
            ...
            //    mdj.ocMostly.tax_ticket = dr["tax_ticket"].ToString();
            ...
            //    mdj.ocMostly.total_package_num = dr["total_package_num"].ToString() == "" ? 0 : Convert.ToDecimal(dr["total_package_num"]);
            ...
            //    mdj.ocMostly.ship_date = Converter.ConvertFieldToCnDataString(dr["ship_date"].ToString());
            ...
            //    mdj.ocMostly.box_no = dr["box_no"].ToString() == "" ? 0 : Convert.ToInt32(dr["box_no"]);
            //    mdj.ocMostly.create_by = dr["create_by"].ToString();
            //    mdj.ocMostly.create_date = Converter.ConvertFieldToCnDateTimeString(dr["create_date"].ToString());
            //    mdj.ocMostly.update_by = dr["update_by"].ToString();
            //    mdj.ocMostly.update_date = Converter.ConvertFieldToCnDateTimeString(dr["update_date"].ToString());
            //    mdj.ocMostly.state = dr["state"].ToString();
            //    mdj.ocMostly.matter = dr["matter"].ToString();
            //    mdj.ocMostly.check_date = Converter.ConvertFieldToCnDateTimeString(dr["check_date"].ToString());
            //    mdj.ocMostly.m_rate = dr["m_rate"].ToString() == "" ? 0 : Convert.ToDecimal(dr["m_rate"]);
            //    mdj.ocMostly.flag = dr["flag"].ToString();
            //    mdj.ocMostly.as_id = dr["as_id"].ToString();
            //    mdj.ocMostly.area = dr["area"].ToString();
            //    mdj.ocMostly.confirm_status = dr["confirm_status"].ToString();

            //    mdj.ocDetails.EditFlag = 0;
            //    mdj.ocDetails.ID = dr["id"].ToString();
            //    mdj.ocDetails.Ver = Convert.ToInt32(dr["Ver"]);
            ...
            //    mdj.ocDetails.shipment_suit = dr["shipment_suit"].ToString();
            //    mdj.ocDetails.table_head = dr["table_head"].ToString();
            //    mdj.ocDetails.goods_name = dr["goods_name"].ToString();
            //    mdj.ocDetails.color = dr["color"].ToString();
            //    mdj.ocDetails.big_class = dr["big_class"].ToString();
            //    mdj.ocDetails.u_invoice_qty = dr["u_invoice_qty"].ToString() == "" ? 0 : Convert.ToDecimal(dr["u_invoice_qty"]);
            //    mdj.ocDetails.goods_unit = dr["goods_unit"].ToString();
            //    mdj.ocDetails.sec_qty = dr["sec_qty"].ToString() == "" ? 0 : Convert.ToDecimal(dr["sec_qty"]);
            //    mdj.ocDetails.sec_unit = dr["sec_unit"].ToString();
            //    mdj.ocDetails.invoice_price = dr["invoice_price"].ToString() == "" ? 0 : Convert.ToDecimal(dr["invoice_price"]);
            //    mdj.ocDetails.p_unit = dr["p_unit"].ToString();
            //    mdj.ocDetails.disc_rate = dr["disc_rate_d"].ToString() == "" ? 0 : Convert.ToDecimal(dr["disc_rate_d"]);
            //    mdj.ocDetails.disc_price = dr["disc_price"].ToString() == "" ? 0 : Convert.ToDecimal(dr["disc_price"]);
            //    mdj.ocDetails.total_sum = dr["total_sum_d"].ToString() == "" ? 0 : Convert.ToDecimal(dr["total_sum_d"]);
            //    mdj.ocDetails.disc_amt = dr["disc_amt_d"].ToString() == "" ? 0 : Convert.ToDecimal(dr["disc_amt_d"]);
            //    mdj.ocDetails.buy_id = dr["buy_id"].ToString();
            //    mdj.ocDetails.order_id = dr["order_id"].ToString();
            //    mdj.ocDetails.issues_id = dr["issues_id"].ToString();
            //    mdj.ocDetails.ref1 = dr["ref1"].ToString();
            //    mdj.ocDetails.ref2 = dr["ref2"].ToString();
            //    mdj.ocDetails.ncv = dr["ncv"].ToString();
            //    mdj.ocDetails.is_print = dr["is_print"].ToString();
            //    mdj.ocDetails.apprise_id = dr["apprise_id"].ToString();
            //    mdj.ocDetails.is_free = dr["is_free"].ToString();
            //    mdj.ocDetails.corresponding_code = dr["corresponding_code"].ToString();
            //    mdj.ocDetails.nwt = dr["nwt"].ToString() == "" ? 0 : Convert.ToDecimal(dr["nwt"]);
            //    mdj.ocDetails.gross_wt = dr["gross_wt"].ToString() == "" ? 0 : Convert.ToDecimal(dr["gross_wt"]);
            //    mdj.ocDetails.package_num = dr["package_num"].ToString() == "" ? 0 : Convert.ToDecimal(dr["package_num"]);
            //    mdj.ocDetails.box_no = dr["box_no"].ToString() == "" ? 0 : Convert.ToInt32(dr["box_no"]);
            //    mdj.ocDetails.subject_id = dr["subject_id"].ToString();
            //    mdj.ocDetails.contract_cid = dr["contract_cid"].ToString();
            //    mdj.ocDetails.location_id = dr["location_id"].ToString();
            //    mdj.ocDetails.customer_goods = dr["customer_goods"].ToString();
            //    mdj.ocDetails.customer_color_id = dr["customer_color_id"].ToString();
            //    mdj.ocDetails.remark = dr["remark"].ToString();
            //    mdj.ocDetails.carton_code = dr["carton_code"].ToString();
            //    mdj.ocDetails.so_ver = dr["so_ver"].ToString() == "" ? 0 : Convert.ToInt32(dr["so_ver"]);
            //    mdj.ocDetails.so_sequence_id = dr["so_sequence_id"].ToString();
            //    mdj.ocDetails.so_bom_sequence = dr["so_bom_sequence"].ToString();
            //    lsInv.Add(mdj);
            //}
        ...
        //public static IList<T> CreateService<T>() where T : new()
        ////public static T CreateService<T>() where T : new()
        //{

        //    //ModelConvertHelper<String> singleton = new ModelConvertHelper<String>();
        //    // 定义集合  
        //    //List<T> ts = new List<T>();
        //    T t = new T();
        //    string result = "";
        //    string strSql = "";
        //    strSql += "Select id AS ID,name AS matter,english_name From cd_port Where within_code='" + within_code + "' Order By id";
        //    //
        //    List<ModelBaseList> ls = new List<ModelBaseList>();
        //    ModelBaseList obj1 = new ModelBaseList();
        //    obj1.value = "";
        ...
        //    var ModelBaseList = ConvertHelper.DataTableToList<T>(dtSalesman); //获取DataTable转换成的泛型集合

        //    var ModelBaseList1 = ModelConvertHelper<InvoiceModel>.DataTableToModel(dtSalesman); //获取DataTable转换成的泛型集合
        //    return ModelBaseList;
        //}

        //public static List<ModelBaseList> DataTableReturnList()
        //{
        //    string strSql = "";
        //    strSql += "Select id AS ID,name AS matter,english_name From cd_port Where within_code='" + within_code + "' Order By id";
        //    //
        ...
        //    {
        //        ModelBaseList obj = new ModelBaseList();
        //        obj.label = dtSalesman.Rows[i]["matter"].ToString();
        //        obj.value = dtSalesman.Rows[i]["ID"].ToString();
        //        ls.Add(obj);
        //    }
        //    return ls;
        //}
